...
| 10 | ~~`frontend/src/components/editor/editor-toolbar.tsx`~~ | **Nothing to do** — the toolbar no longer keeps a per-type reset `switch`. Kept as a numbered row so the steps below don't shift. |
...
| 13 | `frontend/src/components/editor/config-panel.tsx` + `config-panel-label.ts` | Import and render `case` in config-panel.tsx; the display-name fallback lives in config-panel-label.ts |
...
**A parameter picker is a node from a curated catalog with a tile-grid picker UI — Setting, Mood, Action FX, Loop Subject, Person, Lens, Pose, Animal, etc. (the families "Look", "Camera", "Subject / Object" in `packages/picker-ui/src/registry.tsx`). All of these emit a prompt-fragment that gets injected into a downstream node's prompt via FieldMappings — they NEVER make API calls and NEVER produce a job.**
...
3. **`packages/picker-ui/src/registry.tsx` (full registry)** — if missing here but present in #2, `PickerInputCard` finds no meta and renders `null`. Card disappears entirely.
...
| Step | File | What to Update |
|------|------|----------------|
| 20 | `frontend/src/lib/parameter-picker-types.ts` **and** `packages/picker-ui/src/types.ts` | Add the type to BOTH copies of `PARAMETER_PICKER_NODE_TYPES` (the frontend copy is what `input-card.tsx` and every editor consumer import; the picker-ui copy serves the in-repo `@nodaro/picker-ui` workspace package, which is private and never published; no test compares them). Add the node type string to the `PARAMETER_PICKER_NODE_TYPES` set ⚠️ **Without this, `input-card.tsx` falls through to `ParameterCard` (text input) and the picker UI never mounts in apps** |
| 21 | `packages/picker-ui/src/registry.tsx` (+ the matching wiring row in `packages/prompts/src/picker-wiring.ts`) | Add a `kind:"single"` (one valueField + catalog) or `kind:"multi"` (multiple fields + custom Picker) entry. Single: `nodeType`, `label`, `valueField`, `defaultValue`, `catalogId`, `entries: mapCat(CATALOG, "category")`, optional `groupOrder`/`groupLabels`/`renderIcon`. Multi: `fields`, `catalogEntries: flatCat(CATALOG)`, `Picker: erase(YourPickerComponent)`. |
| 21b | `packages/prompts/src/picker-catalogs.ts` | Add the `PICKER_CATALOGS` entry (nodeType, catalogId, valueField, defaultValue, categoryOrder/Labels, `options: toOptions(…)`, optional `dimensions`). Every public surface reads this funnel — `/v1/picker-catalogs`, `/v1/catalogs`, MCP `get_picker_catalog`, `gen:skills` "Valid values", the catalog-id guard, the i18n entry guard — and `parameter-hint-mode.test.ts` fails until the golden fixture has a case (`cd packages/prompts && npx tsx scripts/gen-parameter-hint-golden.ts`). |
...
**Video-only pickers** (their fragment only makes sense in motion) join `VIDEO_ONLY_PARAMETER_NODE_TYPES` in `@nodaro/shared` — both executors and the add-node popup derive their still-image exclusion from it.
...
**Reference example (single-dim):** `loop-subject` — in `packages/picker-ui/src/registry.tsx`, `parameter-picker-types.ts`, `parameter-node-value.ts` (set + `case`), `parameter-prompt-hint.ts`.
**Reference example (multi-dim):** `person` — `packages/picker-ui/src/registry.tsx` + the `PersonPicker` component + `parameter-node-value.ts` `case "person"` (returns first non-empty dimension as the single-string fallback).

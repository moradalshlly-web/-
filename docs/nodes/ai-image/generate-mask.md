...
- Segmentation quality depends on the prompt naming the subject clearly (e.g. `"the left girl's hair"` rather than just `"hair"`). It works well for distinct, clearly-named subjects, but is not perfect: an over-generic prompt or an absent subject can return an empty (fully black) mask -- in that case, rewrite the prompt.
- **When nothing matches at all**, the run fails with *"No region matched the mask prompt. Try a shorter, plainer subject phrase..."* and the credits are refunded. That is a verdict, not a glitch: re-running the same prompt fails the same way. The model reads short noun phrases (`"the hat"`, `"the red car"`, `"the woman"`) far better than long descriptive clauses, so shortening the prompt is usually the fix. The Threshold slider is not -- it has no effect on the pinned model (see the Configuration table).

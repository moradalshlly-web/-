...
- **Segments must fit their sources.** Whether a segment runs past the end of its media can only be known from the file itself, so that is checked once the sources are downloaded — against each source's **real** stream end (measured by reading the file through, not the length its container declares, which an mp3 without a proper header under-reports and a browser recording omits) — and the render **fails, naming the segment and the source**, rather than quietly delivering a shorter cut than the EDL (and its price) describes. An overrun of up to one second is treated as track skew and renders to the source's real end. A file whose length cannot be measured is not checked and renders as it always has.

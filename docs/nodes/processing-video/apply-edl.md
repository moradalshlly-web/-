...
- **Segments must fit their sources.** Whether a segment runs past the end of its media can only be known from the file itself, so that is checked once the sources are downloaded — against the **real end of the track the segment reads** (the picture track for a video render, the sound track always), measured from that track's own packets rather than from the length the container declares (an mp3 without a proper header under-reports it; a browser recording omits it) — and the render **fails, naming the segment, the source and the track**, rather than quietly delivering a shorter cut than the EDL (and its price) describes. An overrun of up to one second is treated as rounding and renders to the track's real end. A track whose end cannot be measured is not checked and renders as it always has.

...
- **Segments must fit their sources.** Whether a segment runs past the end of its media can only be known from the file itself, so that is checked once the sources are downloaded — against the **real end of the track the segment reads** (the picture track for a video render, the sound track always), measured from that track's own timestamps, counted from the start of the file, rather than from the length the container declares (an mp3 without a proper header under-reports it; a browser recording omits it). A violation **fails the job right away, naming the segment, the source and the track** — credits refunded, not retried, since the same edit would fail the same way — rather than quietly delivering a shorter cut than the EDL (and its price) describes. The same happens when a segment's picture source has no video track at all (an audio file, or an mp3 whose only picture is embedded cover art).
- **Every segment renders to its exact length.** A segment that reaches up to one second past its media is treated as rounding: the last frame is held and silence is padded, so the segment keeps its EDL length on both tracks and picture and sound stay in step for every segment after it. A track whose end cannot be measured is not checked (a warning is logged) and renders as it always has.

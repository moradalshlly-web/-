...
| Render method | Select | Extend | **Extend (video chain)** continues each segment from the previous one's tail; **Keyframes (scene anchors)** renders shots from anchor frames and preserves explicitly planned continuous boundaries — see [Render method](#render-method) |
...
**Avoiding a boundary inside an action.** Automatic splitting chooses segment durations before the planner assigns actions to them; faithful planning can therefore split an action across two clips. Preferred segment length influences those durations, but does not detect action boundaries. For a 12-second Gemini Omni Flash sequence containing three 4-second actions, a preferred length of 4 produces three 4-second slots. Unequal action lengths need an explicit `segmentDurations` array through the API/workflow JSON, validated against the selected model's duration menu. Continuous-camera handling preserves the shot across a boundary; it does not relocate that boundary or guarantee the model performs exact timing.
...
- **Continuous shots preserve their boundary frame.** With frame conditioning enabled, the next segment starts from the previous delivered clip's final frame, without redrawing it or requesting a new camera angle. These boundaries render sequentially, including when other anchors are prepared upfront. References-only mode keeps its frame-free behavior; the prompt requests continuity, but no exact frame is pinned.

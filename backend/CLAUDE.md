...
**Private-plugin jobs and the `pre-task` sentinel (2026-09-15):** the video worker stamps `provider_kind="pre-task"` at pickup on EVERY row, and the reconcile cron fails + refunds a `pre-task` row whose stamp is 30 min old. Every handler `loadPrivatePlugins` returns is merged through `withPreTaskHeartbeats` (`workers/pre-task-heartbeat.ts`), which refreshes the stamp every 60 s while the handler runs (capped at `NODE_TIMEOUT_MS`, so a hung handler still ages into the sweep) — a plugin never needs its own heartbeat for this, and a new plugin job type is covered without a list. The refresh (`refreshPreTaskSentinel`) is a CAS on `provider_kind="pre-task"` + `status="processing"` and never writes `provider_kind`: a sentinel a plugin cleared (gvp/evp, owned by the 90-min orchestrator sweep) stays null, and a real provider kind is never overwritten. Do NOT "fix" a long run by raising `STALE_THRESHOLD_MS["pre-task"]` — it is the dead/hung backstop for every video-worker job type. History: staging Pro 3D Render job 99ede351 was failed at minute 31 with its worker alive (its only heartbeat renewed a Redis stage lease).

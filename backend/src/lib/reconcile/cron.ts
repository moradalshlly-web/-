 ...
 *  sweep. The host's pre-task heartbeat — applied to every handler the video
 *  worker dispatches (`workers/pre-task-heartbeat.ts`) — is a CAS on
 *  `provider_kind = "pre-task"`, so it leaves a cleared sentinel null and
 *  these rows on this sweep.
 *

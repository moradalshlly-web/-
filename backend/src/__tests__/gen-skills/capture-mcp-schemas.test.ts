...
describe("captureMcpToolSchemas", () => {
  it("captures generate_image schema with prompt field", { timeout: 20_000 }, async () => {
  ...
  it("captures animate_image and generate_music tools", { timeout: 20_000 }, async () => {
    const schemas = await captureMcpToolSchemas()
    expect(schemas.find((s) => s.name === "animate_image")).toBeDefined()
    expect(schemas.find((s) => s.name === "generate_music")).toBeDefined()
  })

  it("captures get_node_skill and start_workflow_editor (Phase A tools)", { timeout: 20_000 }, async () => {
    const schemas = await captureMcpToolSchemas()
    expect(schemas.find((s) => s.name === "get_node_skill")).toBeDefined()
    expect(schemas.find((s) => s.name === "start_workflow_editor")).toBeDefined()
  })

  it("returns at least 30 tools (the full registered set)", { timeout: 20_000 }, async () => {
    const schemas = await captureMcpToolSchemas()

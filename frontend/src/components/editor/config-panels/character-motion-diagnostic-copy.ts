...
export function formatCharacterMotionDiagnostic(
  ...
  switch (item.code) {
    ...
    case "omitted": return t(item.severity === "error" ? "motionReview.diagnostic.omittedAll" : "motionReview.diagnostic.omitted", { motions: item.ids.map(label).join(", ") })
    case "multiple-targets": return t("motionReview.diagnostic.multipleTargets")
    case "roles": return t("motionReview.diagnostic.roles")
    case "retired": return [t("motionReview.diagnostic.retired", { motion }), entry?.replacementId ? t("motionReview.diagnostic.replacement", { motion: label(entry.replacementId) }) : ""].filter(Boolean).join(" ")
    ...
    case "requirements": return [
      ...
      entry?.twoPerson || entry?.counterpart
        ? bindings.partnerNames?.length ? t("motionReview.diagnostic.partnerReference", { names: bindings.partnerNames.join(", ") }) : t("motionReview.diagnostic.partnerFallback")

...
DO $$ DECLARE id uuid; w jsonb; again jsonb; BEGIN
 id:=public.reserve_credits('00000000-0000-4000-8000-00000000e001',100,NULL,'test-wallet',p_on_behalf_of=>'00000000-0000-4000-8000-00000000e002');
 w:=public.prepare_external_wallet(id,'00000000-0000-4000-8000-00000000e002','00000000-0000-4000-8000-00000000e001','sai');
 again:=public.prepare_external_wallet(id,'00000000-0000-4000-8000-00000000e002','00000000-0000-4000-8000-00000000e001','sai');
 ...
 PERFORM pg_temp.wallet_refused(format('SELECT public.commit_credits(%L,40)',id));
 PERFORM pg_temp.wallet_refused(format('SELECT public.prepare_external_wallet(%L,%L,%L,%L)',id,'00000000-0000-4000-8000-00000000e001','00000000-0000-4000-8000-00000000e001','sai'));
 PERFORM pg_temp.wallet_assert(public.authorize_external_wallet(id),'authorization persisted');
 PERFORM pg_temp.wallet_assert(public.abort_external_wallet(id),'late abort cannot cancel authorized work');
 PERFORM public.commit_credits(id,40);
 PERFORM pg_temp.wallet_assert((SELECT actual_credits=40 AND delivered_at IS NULL FROM public.external_wallet_operations WHERE usage_log_id=id),'partial charge queued atomically');
 ...
 PERFORM public.commit_credits(id,40);
 PERFORM pg_temp.wallet_assert((SELECT count(*)=1 FROM public.external_wallet_operations WHERE usage_log_id=id),'settlement replay creates no duplicate');
 PERFORM pg_temp.wallet_refused(format('UPDATE public.usage_logs SET credits_charged=41 WHERE id=%L',id));
 PERFORM pg_temp.wallet_refused(format('UPDATE public.usage_logs SET status=%L WHERE id=%L','reserved',id));
 RAISE NOTICE 'ok authorization, immutable identity, partial settlement and replay';
END $$;
DO $$ DECLARE id uuid; BEGIN
 id:=public.reserve_credits('00000000-0000-4000-8000-00000000e001',100,NULL,'test-wallet',p_on_behalf_of=>'00000000-0000-4000-8000-00000000e002');
 PERFORM public.prepare_external_wallet(id,'00000000-0000-4000-8000-00000000e002','00000000-0000-4000-8000-00000000e001','sai');
 PERFORM pg_temp.wallet_assert(NOT public.abort_external_wallet(id),'unknown reserve is cancelled');
 PERFORM pg_temp.wallet_assert(NOT public.authorize_external_wallet(id),'late approval cannot dispatch');
 PERFORM pg_temp.wallet_assert((SELECT actual_credits=0 AND delivered_at IS NULL FROM public.external_wallet_operations WHERE usage_log_id=id),'terminal tombstone queued');
 ...
 PERFORM public.abort_external_wallet(id);
...
SET LOCAL ROLE service_role;
DO $$ DECLARE id uuid; BEGIN
 id:=public.reserve_credits('00000000-0000-4000-8000-00000000e001',10,NULL,'test-wallet',p_on_behalf_of=>'00000000-0000-4000-8000-00000000e002');
 PERFORM pg_temp.wallet_refused(format('SELECT public.prepare_external_wallet(%L,%L,%L,%L)',id,'00000000-0000-4000-8000-00000000e002','00000000-0000-4000-8000-00000000e001','sai'));
 PERFORM public.abort_external_wallet(id);
